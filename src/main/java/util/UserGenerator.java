...
public class UserGenerator {
    ...
    public static String getRandomPassword() {
        return PASSWORD + faker.internet()
                .password(CHARACTERS_8, CHARACTERS_12, true, true, true);
    }

    @Step("Generate random email user default")
    public static String getRandomEmail() {
        return EMAIL + faker.internet()
                .emailAddress();
    }

    @Step("Generate random user default")
    public static User getRandomUser() {
        String login = NAME + faker.name()
                .username();
        String password = PASSWORD + faker.internet()
                .password(CHARACTERS_8, CHARACTERS_12, true, true, true);
        String email = EMAIL + faker.internet()
                .emailAddress();
    ...
    public static User getRandomShortPasswordUser() {
        String login = NAME + faker.name()
                .username();
        String password = PASSWORD + faker.internet()
                .password(CHARACTERS_3, CHARACTERS_5, true, true, true);
        String email = EMAIL + faker.internet()
                .emailAddress();
        return new User(login, password, email);
    }
